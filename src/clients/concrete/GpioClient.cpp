...
GpioClient::GpioClient(bool sim) : m_useSimulatedGPIO(sim) {
	...
	for(auto& [buttonID, ButtonName] : ButtonToName)
		addButtonCallback(buttonID, "DBG" + ButtonName, [&]() { std::cout << "TestButton " << ButtonName << std::endl; });
}
...
void GpioClient::addButtonCallback(BUTTON btn, const std::string& callbackName, const std::function<void()>& callback) {
	std::lock_guard<std::mutex> lck{m_lock};
