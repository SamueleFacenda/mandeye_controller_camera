...
LivoxClient::LivoxClient()
	: imuIteratorToFileSaver("csv",
							 "imu",
							 [](const LivoxIMU& imu) {
								 std::stringstream ss;
								 ss << imu.timestamp << " " << imu.point.gyro_x << " " << imu.point.gyro_y << " " << imu.point.gyro_z << " "
									<< imu.point.acc_x << " " << imu.point.acc_y << " " << imu.point.acc_z << " " << imu.laser_id;
								 return ss.str();
							 })
	, lidarIteratorToFileSaver(
		  "lidar", "ls", [](const std::pair<const uint32_t, std::string>& id_sn) { return std::to_string(id_sn.first) + " " + id_sn.second; }) {
	m_timestamp = -1;
}
...
void LivoxClient::setTimestamp(uint64_t ts, LivoxClient* this_ptr) {
	...
	if(this_ptr->m_timestamp == -1) {
		this_ptr->systemTimestampDelay =
			std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - ts;
	}
